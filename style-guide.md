...
- **Praise-as-structure cluster (very high signal):** treating a sentence, phrase, post, or tweet as if it were structural engineering. "load-bearing" (as analytical metaphor), "X is doing the heavy lifting," "do/does/did/doing real work," "the post/sentence deserves the weight," "I keep coming back to this [tweet/post/line]." These almost always identify the writer as Claude. Just say what the thing does. (One narrow exception: `load-bearing assumption` in the Assumption-Based Planning sense, capped at two uses per paper. See the carve-out under AI vocabulary below.)
...
- Avoid AI vocabulary, identified empirically by expert human detectors of AI-generated text (Russell, Karpinska, Iyyer 2025, arXiv:2501.15654):
  ...
  - **`load-bearing assumption` is a citable term of art, capped at two per paper.** Assumption-Based Planning (Dewar et al. 1993) defines a load-bearing assumption as one whose failure would mean the plan fails to meet its objectives; the term is standard in the decision-making-under-deep-uncertainty literature (Marchau et al. 2019). That sense is permitted. Three conditions, all of them: it appears only in the exact collocation `load-bearing assumption`, never bare and never modifying a sentence, claim, argument, post, or section; first use carries a gloss and the citation; and it appears **at most twice in a paper**. Elsewhere write `critical assumption`, or name the assumption. The cap is tighter than the `robust` carve-out above because the two words sit differently in the corpus: `robust` already has 670 technical uses to protect, whereas `load-bearing` has no established use here and would be newly imported into a house style that bans it as a very-high-signal tell. A term human detectors read as an LLM tic does not stop reading that way because this particular use is correct, so the technical sense has to stay rare enough to look like citation rather than habit. Every other use stays banned under the praise-as-structure cluster.
